 ...
 * @generated
 */
public class ScxmlDiagramEditorUtil {
	...
	*/
	public static boolean openDiagram(Resource diagram) throws PartInitException {
		...
		page.openEditor(new URIEditorInput(diagram.getURI()), ScxmlDiagramEditor.ID);
		return true;
	}
	...
	*/
	...
	public static Resource openModel(Shell shell, String description, TransactionalEditingDomain editingDomain) {
		...
		try {
			...
			ScxmlDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + uri, we); //$NON-NLS-1$
			MessageDialog.openError(shell, Messages.ScxmlDiagramEditorUtil_OpenModelResourceErrorDialogTitle,
					NLS.bind(Messages.ScxmlDiagramEditorUtil_OpenModelResourceErrorDialogMessage, fileName));
		}
		return resource;
	}
	...
	public static void runWizard(Shell shell, Wizard wizard, String settingsKey) {
		IDialogSettings pluginDialogSettings = ScxmlDiagramEditorPlugin.getInstance().getDialogSettings();
	...
	*/
	public static Resource createDiagram(URI diagramURI, URI modelURI, IProgressMonitor progressMonitor) {
		...
		progressMonitor.beginTask(Messages.ScxmlDiagramEditorUtil_CreateDiagramProgressTask, 3);
		final Resource diagramResource = editingDomain.getResourceSet().createResource(diagramURI);
		final Resource modelResource = editingDomain.getResourceSet().createResource(modelURI);
		final String diagramName = diagramURI.lastSegment();
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(editingDomain,
				Messages.ScxmlDiagramEditorUtil_CreateDiagramCommandLabel, Collections.EMPTY_LIST) {
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
					throws ExecutionException {
				ServiceTemplate model = createInitialModel();
				attachModelToResource(model, modelResource);

				Diagram diagram = ViewService.createDiagram(model, ServiceTemplateEditPart.MODEL_ID,
						ScxmlDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
				if (diagram != null) {
				...
				try {
					modelResource.save(scxml.diagram.part.ScxmlDiagramEditorUtil.getSaveOptions());
					diagramResource.save(scxml.diagram.part.ScxmlDiagramEditorUtil.getSaveOptions());
				} catch (IOException e) {

					ScxmlDiagramEditorPlugin.getInstance().logError("Unable to store model and diagram resources", e); //$NON-NLS-1$
				}
	...
	*/
	private static ServiceTemplate createInitialModel() {
		return ScxmlFactory.eINSTANCE.createServiceTemplate();
	}
	...
	* @generated
	*/
	private static void attachModelToResource(ServiceTemplate model, Resource resource) {
